...
El parámetro `--exclusive` es opcional, y sólo deber ser utilizado si se pretende reservar todo el nodo, lo que implica bloquear el acceso a los demás usuarios. 
...
Lo primero que se tiene que hacer antes de trabajar con un GPU, es obtener información acerca de la GPU y su actividad.
...
```
[ronald.gualan@compute-0-12 ~]$ cd /usr/local/cuda/samples/1_Utilities/deviceQuery
[ronald.gualan@compute-0-12 deviceQuery]$ ./deviceQuery
...
La salida del comando anterior cuando los GPUs están desocupados es la siguiente: 
...
La salida cuando los GPUs están desocupados: 
...
Los cálculos clave se realizan en el *kernel*. Los *kernel* son funciones que codifican las operaciones de cómputo centrales que se ejecutan en paralelo. 
...
for(order in orders) {
  ...
  elapsed = system.time({gpuC = gpuA %*% gpuB})[3]
  t1 = c(t1, elapsed)
}
...
for(order in orders) {
  A = matrix(rnorm(order^2), nrow=order) 
  B = matrix(rnorm(order^2), nrow=order) 
  elapsed = system.time({C = A %*% B})[3] 
  t2 = c(t2, elapsed)
}

df = data.frame(order=orders, speedup=t2/t1)
ggplot(r, aes(factor(order), speedup)) + 
